(function () {
  ...
  function renderUserSnippets() {
    ...
    list.querySelectorAll('.usnippet-insert').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (text) { sendToContent({ type: 'INSERT_TEXT', text }); toast('已插入', 'success'); }
      });
    });
  ...
  function showSettingsModal() {
    ...
    $('modalBody').innerHTML = `
      ...
      <div class="form-row">
        <label>启用站点关键词</label>
        <small class="form-tip">URL 包含以下任一关键词时启用插件（每行一个）</small>
        <textarea id="setSitesInput" rows="4"></textarea>
      </div>
      <div class="form-row">
    ...
    loadState().then(() => {
      const resp = chrome.storage.local.get(STORAGE_KEYS.ENABLED_SITES, (d) => {
